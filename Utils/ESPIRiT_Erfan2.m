function sss = ESPIRiT_Erfan2(DATA,CalibSize,ksize,eigThresh_1)
...
% load PLORAKS_normalized_test_image_4coils
% DATA = fft2c(x0);


% %load IMG_bssfp
% load Bilgic
% x = double(imresize(x0(:,:,1),[80 80]));
% load Sense8_cplx_80
% 
% for i = 1:8
% DATA(:,:,i) = fft2c(x.*ss(:,:,i));
...
% % load Knee_Qu
% load Itthi
% x0 = cat(3,2.5*x0(:,:,1),3*x0(:,:,2),5*x0(:,:,3)); %% Only uncommnet for Itthi
% %load Breast
% %x0 = flip(x0);
% x0 = imresize(x0,[n n]);
% load Sense8_2_cplx_200
% %  
%   i = 1;
%  for j = 1:size(ss,3)
%        DATA(:,:,j) = fft2c(ss(:,:,j).*...
...
%% off-the-shelf Data
% load data_slice10_4coil_cut
% im_ori = imresize(im_ori,[200 200]);
% DATA = fft2c(im_ori);
 
%%
[sx,sy,Nc] = size(DATA);
% ncalib = 28; % use 24 calibration lines to compute compression
...
% figure, imshow3(abs(maps),[],[1,Nc]); 
% title('Absolute sensitivity maps');
% colormap((gray(256))); colorbar;
% 
% figure, imshow3(angle (maps),[],[1,Nc]); 
